...
if(isset($_GET['id']))
{
	$rs=mysql_query("Select * from table_login where sha1(username)='".$_GET['id']."'");
...
if(isset($_GET['edit']))
{
	...
	$rs=mysql_query("Update table_login SET password='".$_POST['password']."',login_hash='".$_POST['jenis']."' Where sha1(username)='".$_GET['id']."'");
	if($rs)
	{
		echo "<script>window.location='?cat=administrator&page=user'</script>";
	}

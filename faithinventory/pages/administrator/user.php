...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-striped">
  ...
  <?php
  $rw=mysql_query("Select * from table_login");
...
if(isset($_GET['act']))
{
	...
	$rs=mysql_query("Insert into table_login (`username`,`password`,`login_hash`) values ('".$_POST['username']."','".$_POST['password']."','".$_POST['jenis']."')") or die(mysql_error());
	if($rs)
	{
...
{
	$ids=$_GET['id'];
	$ff=mysql_query("Delete from table_login Where sha1(username)='".$ids."'");
	if($ff)
	{
		echo "<script>window.location='?cat=administrator&page=user'</script>";
	}
